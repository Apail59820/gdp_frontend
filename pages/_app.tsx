...
export default function App({ Component, pageProps }: AppProps) {
  ...
  const [notificationsProps, setNotificationsProps] =
    useState<TopBarNotificationProp>({
      messages: [],
      amount: 0,
      ids: [],
      onMarkAsRead: null,
  ...
  async function updateNotifications() {
    ...
    getGdpUsersNotifications({
      filter: {
        _and: [
          { seen: { _eq: false }, directus_users_id: myUsProfile?.data.id },
        ],
      },
  ...
  const markAsRead = async (id: number) => {
    updateGdpUsersNotifications({ keys: [id], data: { seen: true } })
      .then(async () => {
            ...
            <header id="header">
              <TopBar
                items={[
                  ...
                  {
                    label: "Catalogue des solutions alternatives",
                    href: `${publicRuntimeConfig.CATALOG_APP_URL}/`,
                    active: false,
                  },
                ]}
                user={user}
