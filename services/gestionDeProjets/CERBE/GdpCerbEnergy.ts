...
type updateFieldsToOmit =
  ...
  | "date_updated"
  | "affairs_id";
export async function updateGdpCerbEnergy(
  id: number,
  data: Partial<Omit<GdpCerbEnergyModel, updateFieldsToOmit>>,
): Promise<{
  status: number;
  data?: Partial<GdpCerbEnergyModel>;
  error?: string;
}> {
  const token = await retrieveToken();
  if (!token) return Promise.resolve({ status: 401 });
  ...
  return fetch(
    `${publicRuntimeConfig.GESTION_DE_PROJET_API_URL}/items/cerb_energy/${id}`,
    myInit,
  )
    .then(async (response) => {
...
export function useUpdateGdpCerbEnergy() {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: (updatedEnergy: Partial<GdpCerbEnergyModel>) => {
      return updateGdpCerbEnergy(updatedEnergy?.id as number, updatedEnergy);
    },
    onSuccess: (data) => {
      if (isRequestSuccessful(data.status)) {
        queryClient.invalidateQueries({ queryKey: ["cerb_energy"] });
      }
    },
  });
}
