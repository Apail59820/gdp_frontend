...
export async function getUsUsers(
  ...
  return fetch(`${publicRuntimeConfig.USER_SERVICE_API_URL}/items/users?${concatenateQueryParameters(props)}`, myInit)
    .then((res) => {
      if (res.status == 200)
...
export async function getUsUser(
  ...
  return fetch(`${publicRuntimeConfig.USER_SERVICE_API_URL}/items/users/${id}?fields=${fields}`, myInit).then((res) => {
    if (res.status === 200) {
      return res.json().then((data) => {
        return { status: res.status, data: data.data };
      });
 ...
 */
export async function updateUsUser(payload: {
  keys: number[];
  ...
  return fetch(`${publicRuntimeConfig.USER_SERVICE_API_URL}/items/users`, myInit)
    .then((response) => {
